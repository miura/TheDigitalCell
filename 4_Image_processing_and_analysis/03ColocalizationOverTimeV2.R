...
plot(0,0, type="l", xlim=c(0,fr), ylim=c(0.8,0.95), xlab="Frame", ylab="SRCC")
lines(1:fr, r1, lwd=2)
# lines(1:fr, r2, col="gray", lwd=2)
# legend("topleft",legend=c("c1 vs c2", "randomized"), bty = "n", col=c("black", "gray"), lwd=2)
legend("topleft",legend=c("c1 vs c2"), bty = "n", col=c("black"), lwd=2)
dev.off()

# 無作為化したものとのSRCCも合わせてプロットする
png("corr_compare.png")
par(cex = 1.5, mar = c(4, 4, 1, 1))
plot(0,0, type="l", xlim=c(0,fr), ylim=c(-0.1,1.2), xlab="Frame", ylab="SRCC")
lines(1:fr, r1, lwd=2)
lines(1:fr, r2, col="gray", lwd=2)
